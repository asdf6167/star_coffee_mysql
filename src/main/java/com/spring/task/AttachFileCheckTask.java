...
public class AttachFileCheckTask {
	...
	public void checkFiles() throws Exception{
		...
		//	Path path = Paths.get("C:\\upload", vo.getUploadPath(), "s_" + vo.getUuid() + "_" + vo.getFileName());
			Path path = Paths.get("/usr/local/tomcat8/upload", vo.getUploadPath(), "s_" + vo.getUuid() + "_" + vo.getFileName());
			checkFilePath.add(path);
		});
		
		//디렉토리 파일 리스트
	//	File targetDir = Paths.get("C:\\upload", getFolderYesterDay()).toFile();
		File targetDir = Paths.get("/usr/local/tomcat8/upload", getFolderYesterDay()).toFile();
